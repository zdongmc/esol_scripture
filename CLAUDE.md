...
- **Verification status:** en, es, zh-tw, pt, it, fr, am, uk, ar, be, zh-cn, fa verified verse-by-verse. The other five (hi, te, ko, vi, ta) have **not** been — see `SOURCES.md`.
- **Licensing:** Belarusian (Bokun) is CC BY-ND 4.0 — it may not be modified. Simplified Chinese (CCB) is CC BY-SA 4.0. Both require fuller attribution than the `BEL` / `CCB` labels currently give. Every other verified language is public domain or permissive. See `SOURCES.md`.
